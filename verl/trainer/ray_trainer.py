...
class RayPPOTrainer:
    ...
    def _validate(self) -> dict[str, Any]:
        reward_tensor_lst = []
        test_reward_tensor_lst = []
        # Lists to collect samples for the table
        sample_inputs, sample_outputs, sample_labels, sample_scores = [], [], [], []
        test_sample_inputs, test_sample_outputs, test_sample_labels, test_sample_scores = [], [], [], []
        reward_metrics_lst = defaultdict(list)
        length_metrics_lst = defaultdict(list)
        test_reward_metrics_lst = defaultdict(list)
        test_length_metrics_lst = defaultdict(list)
        ...
        for batch_dict in self.test_dataloader:
            test_batch = DataProto.from_single_dict(batch_dict)
            test_gen_batch = test_batch.pop(
                batch_keys=["input_ids", "attention_mask", "position_ids"],
                non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data"],
            ...
            test_sample_labels.extend(test_batch.non_tensor_batch["ground_truth"].tolist())
            test_sample_scores.extend(scores)

            test_reward_tensor_lst.append(reward_tensor)
            for key, value in reward_metrics.items():
                test_reward_metrics_lst[key].extend(value)

            for key, value in compute_length_metrics(test_batch).items():
                test_length_metrics_lst[key].append(value)
        ...
        test_length_metrics = {f"test_{key}": value for key, value in reduce_metrics(test_length_metrics_lst).items()}
        print("Finish validation.")
        return {"val/reward_score": self.val_reward_score, **val_reward_metrics, **val_length_metrics, "test/reward_score": self.test_reward_score, **test_reward_metrics, **test_length_metrics}
