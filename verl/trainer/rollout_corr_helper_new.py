...
def compute_rollout_correction_weights(
    ...
    rollout_is_alpha: float = 1.0, # Alpha for Power-IS scaling
) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    Args:
        ...
        rollout_is_batch_normalize: Whether to normalize IS weights to have mean=1.0 per batch,
            default False.
        rollout_is_alpha: Power factor for IS weights to smooth distribution during cold-start.
    ...
    # Apply Alpha-Power scaling (Power-IS) right before masking and normalization
...
def compute_rollout_correction_and_rejection_mask(
    ...
    rollout_is_batch_normalize: bool = False,
    rollout_is_alpha: float = 1.0, # Add alpha support
    ...
    Args:
        ...
        rollout_is_batch_normalize: Whether to normalize IS weights to have mean=1.0 per batch.
            Default: False.
        rollout_is_alpha: Power scaling for IS weights.
    ...
    if rollout_is is not None and rollout_is_threshold is not None:
        rollout_is_weights, is_metrics = compute_rollout_correction_weights(
            ...
            rollout_is_alpha=rollout_is_alpha,
        )
